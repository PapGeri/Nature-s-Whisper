...
class HeaderBar extends React.Component<HeaderBarProps, HeaderBarState> {
	render() {
		const currentUserAvailable: User | null = getAuth().currentUser;
		return (
			<AppBar position='static' color='transparent'>
				<Toolbar variant='dense'>
					...
					{!!currentUserAvailable ? (
						<Typography>
							{'Welcome ' + currentUserAvailable!.displayName}
						</Typography>
					) : (
						<Button onClick={this.props.onSignInButtonPressed} color='inherit'>
							Sign In
						</Button>
					)}

					<Button onClick={this.props.onSignOutButtonPressed} color='inherit'>
						Log Out
					</Button>
					<IconButton
						color='secondary'
